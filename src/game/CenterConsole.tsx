      ...
      <svg viewBox={`0 0 ${w} ${h}`} width={w} height={h} className="console-svg">
        <defs>
          <linearGradient id="console-bg" x1="0" y1="0" x2="0" y2="1">
            ...
            <stop offset="1" stopColor="#0c0819" />
          </linearGradient>
          <filter id="console-glow" x="-50%" y="-50%" width="200%" height="200%">
            <feGaussianBlur stdDeviation="4" />
          </filter>
        </defs>
        <rect x={4} y={10} width={w - 8} height={h - 8} rx={34} fill="#000" opacity={0.45} />
        <rect x={4} y={4} width={w - 8} height={h - 8} rx={34} fill="url(#console-bg)" stroke={accent} strokeWidth={3} />
        <rect x={12} y={12} width={w - 24} height={h - 24} rx={27} fill="none" stroke="#fff" strokeOpacity={0.1} strokeWidth={1.5} />
        ...
        {view.seats.map((s, seat) => {
          ...
          const color = acting ? '#6bf2c1' : active ? accent : shade(accent, -0.6);
          return (
