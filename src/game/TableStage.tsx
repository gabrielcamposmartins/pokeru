...
function Board({ board, highlight }: { board: Card[]; highlight: Card[] }) {
...
function OpponentCards({ view, geo }: { view: TableView; geo: SeatGeo[] }) {
  return (
    <>
      {view.seats.map((s, seat) => {
        ...
        return s.cards.map((c, i) => {
          ...
          return (
            ...
            >
              <CardView card={c} faceUp={!!c} width={w} back={s.cosmetics.back} highlight={view.highlight.length > 0 && isHl(view.highlight, c)} />
            </div>
          );
        });
      })}
...
function MyHand({ view }: { view: TableView }) {
      ...
      <AnimatePresence>
        {me &&
          cards.map((c, i) => (
            ...
            >
              <CardView card={c} faceUp={!!c} width={MY_CARD_W} back={me.cosmetics.back} highlight={hl.length > 0 && isHl(hl, c)} dim={hl.length > 0 && !isHl(hl, c)} />
            </motion.div>
          ))}
      </AnimatePresence>
    </div>
  ...
  return (
    <div className="table-stage">
      ...
      <div className="table-plane" style={PLANE}>
        ...
        <Board board={view.board} highlight={view.highlight} />
        <OpponentCards view={view} geo={geo} />
