 ...
 * números) no banco de dados libSQL (`POKERU_DB_URL`) ou, sem ele, num arquivo JSON.
 *
 ...
 *   POKERU_DB_URL=…          banco libSQL onde as contas moram (ex. http://db:8080, o da VM). Na
 *                            primeira subida importa o accounts.json que houver em DATA_DIR.
 *   POKERU_DB_TOKEN=…        token do banco, se ele exigir
 *
...
/** As opções das contas que não dependem de onde elas moram. */
const OPCOES_DAS_CONTAS = {
...
/**
 * Abre as contas: no banco de dados quando `POKERU_DB_URL` está definida (o libSQL da VM), senão
 * no arquivo `accounts.json` de sempre.
 *
 * Com o banco, a primeira subida importa o `accounts.json` que houver (e o renomeia para
 * `.importado`). Se o banco não responder, a subida falha de propósito: subir vazio deixaria cada
 * jogador entrar sem a conta dele, e o Docker tenta de novo sozinho.
 */
async function abrirContas(): Promise<Accounts | null> {
  if (!WITH_ACCOUNTS) return null;
  const url = process.env.POKERU_DB_URL || '';
  if (!url) return new Accounts({ ...OPCOES_DAS_CONTAS, file: dataFile('accounts.json') });
  const { createClient } = await import('@libsql/client/web');
  const db = createClient({ url, authToken: process.env.POKERU_DB_TOKEN || undefined });
  const store = await ContasNoBanco.abrir({ db, importarDe: dataFile('accounts.json') });
  return new Accounts({ ...OPCOES_DAS_CONTAS, store });
}
...
/**
 * Sobe o servidor com as contas já abertas.
 *
 * Tudo daqui para baixo precisa das contas, e abrir o banco é assíncrono (é ida à rede): por isso
 * o lobby, as rotas e as conexões nascem aqui dentro, depois dele.
 */
function iniciar(accounts: Accounts | null): void {
  ...
  function status() {
    return {
      ...
      persistence: accounts ? (process.env.POKERU_DB_URL ? 'libsql' : 'json') : 'off',
  ...
  http.listen(PORT, () => {
    ...
    console.log(accounts ? `   contas: ${accounts.count} em ${accounts.onde}` : '   contas desligadas (mesas livres)');
    if (auth) {
  ...
  function shutdown(signal: string): void {
    ...
    wss.close();
    // as contas gravam por último (com o banco, espera a gravação terminar) e só então o processo sai
    void Promise.resolve(accounts?.close())
      .catch((err: unknown) => console.error('[dados] falha ao gravar no desligamento', err))
      .finally(() => http.close(() => process.exit(0)));
    // não deixa o processo pendurado se algum socket (ou o banco) travar
...
abrirContas().then(iniciar, (err: unknown) => {
  console.error(`\n\u2660 não consegui abrir as contas: ${err instanceof Error ? err.message : err}\n`);
  process.exit(1);
});
